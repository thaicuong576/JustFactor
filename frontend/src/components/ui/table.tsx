...
>(({ className, ...props }, ref) => (
    <th
        ref={ref}
        className={cn(
            "h-12 px-4 text-left align-middle text-xs font-black uppercase tracking-[0.1em] text-slate-500 [&:has([role=checkbox])]:pr-0",
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <td
        ref={ref}
        className={cn("p-4 align-middle font-medium text-slate-700 [&:has([role=checkbox])]:pr-0", className)}
        {...props}
    />
))
